 ...
 * @Date: 2026-01-15 16:15:01
 * @LastEditors: liusuxian 382185882@qq.com
 * @LastEditTime: 2026-01-15 18:42:01
 * @Description:
 *
...
// memoryBatchSetter MemoryCache 批量设置构建器
type memoryBatchSetter struct {
	mc             *memoryCache
	items          []batchSetItem
...
//	返回构建器自身，支持链式调用
func (bs *memoryBatchSetter) Add(ctx context.Context, key string, val any, timeout ...time.Duration) (batchSetter IBatchSetter) {
...
//	返回构建器自身，支持链式调用
func (bs *memoryBatchSetter) SetDefaultTimeout(ctx context.Context, timeout time.Duration) (batchSetter IBatchSetter) {
	if timeout > 0 {
...
//	建议：为每次批量操作创建新的构建器实例
func (bs *memoryBatchSetter) Execute(ctx context.Context) (err error) {
	if len(bs.items) == 0 {
	...
	for _, item := range bs.items {
		...
		if timeout != nil && *timeout > 0 {
			...
			oldItem, found := bs.mc.items[item.key]
			if found && !oldItem.isExpired() {
				expiration = oldItem.Expiration
			}
		}

		bs.mc.items[item.key] = &Item{
			Object:     item.val,
