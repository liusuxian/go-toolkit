 ...
 * @LastEditTime: 2024-01-24 23:05:54
 * @Description:
 *
...
func TestKafka(t *testing.T) {
	...
	// 创建 kafka 客户端
	kafkaClient := gtkkafka.NewClient(func(cc *gtkkafka.ClientConfig) {
		cc.IsClose = true
		cc.Env = "test"
	...
	if err = kafkaClient.NewProducer("test"); err != nil {
		t.Fatal("NewProducer Error: ", err)
	}
	if err = kafkaClient.NewConsumer("test"); err != nil {
		t.Fatal("NewConsumer Error: ", err)
	}
	if err = kafkaClient.SendJsonData("test", TestRecordInfo{
		Uid:  1111,
		Sid:  2222,
