...
func (kc *KafkaClient) GetTopicPartitionNum(producerConsumerName string) (partitionNum uint32, err error) {
	...
	err = gerror.Newf("ProducerConsumerName `%s` Not Found", producerConsumerName)
	return
}
...
func (kc *KafkaClient) NewProducer(ctx context.Context, producerConsumerName string) (err error) {
	...
	if kc.config.IsClose {
		gflogger.Infof(ctx, "init kafka producer: %s, topic: %s, originTopic: %s success (isClosed)", producerName, topic, originTopic)
		return
	}
	...
	if producer == nil {
		err = gerror.Newf("NewProducer %s Failed", producerName)
		return
	}
		...
		for e := range producer.Events() {
			...
			case *kafka.Message:
				if ev.TopicPartition.Error != nil {
					gflogger.Errorf(ctx, "%s producer failed to write access log entry: %v, topic: %v, partition: %v, offset: %v, key: %s, content: %s",
					...
					gflogger.Debugf(ctx, "%s producer send ok topic: %v, partition: %v, offset: %v, key: %s, content: %s",
	...
	gflogger.Infof(ctx, "init kafka producer: %s, topic: %s, originTopic: %s success", producerName, topic, originTopic)
	return
}
...
func (kc *KafkaClient) NewConsumer(ctx context.Context, producerConsumerName string) (err error) {
	...
	if kc.config.IsClose {
		gflogger.Infof(ctx, "init kafka consumer: %s, topic: %s, group: %s, partitionNum: %v success (isClosed)", consumerName, topic, group, partitionNum)
		return
	}
	var kafkaCnf = &kafka.ConfigMap{
		"api.version.request":       "true",
	...
	for i := 0; i < int(partitionNum); i++ {
		...
		if consumer == nil {
			return gerror.Newf("NewConsumer %s Failed", consumerName)
		}
		consumerList = append(consumerList, consumer)
	}

	kc.consumerMap[consumerName] = consumerList
	gflogger.Infof(ctx, "init kafka consumer: %s, topic: %s, group: %s, partitionNum: %v success", consumerName, topic, group, partitionNum)
	return
}
...
func (kc *KafkaClient) SubscribeTopics(ctx context.Context, producerConsumerName string, fn func(message *kafka.Message) error) (err error) {
	...
	topics := []string{topic}
	if kc.config.IsClose {
		gflogger.Infof(ctx, "subscribeTopics consumer: %s, topics: %v (isClosed)", consumerName, topics)
		return
	}
	// 订阅topics
	...
	if consumerList, ok = kc.consumerMap[consumerName]; !ok {
		err = gerror.Newf("consumer: %s, topics: %v not found", consumerName, topics)
		return
	}
	for _, v := range consumerList {
		...
		go func(consumer *kafka.Consumer) {
			...
			defer func() {
				if r := recover(); r != nil {
					gflogger.Errorf(ctx, "%s %v consumer panic: %v", consumerName, consumer, r)
				}
			}()
					...
					if pErr == nil {
						...
						if kafkaErr, ok := pErr.(kafka.Error); ok {
							if kafkaErr.Code() != kafka.ErrTimedOut {
								gflogger.Errorf(ctx, "%s %v consumer error: %v, msg: %+v", consumerName, consumer, pErr, msg)
							}
						} else {
							gflogger.Errorf(ctx, "%s %v consumer error: %v, msg: %+v", consumerName, consumer, pErr, msg)
						}
...
func (kc *KafkaClient) BatchSubscribeTopics(ctx context.Context, producerConsumerName string, fn func(messages []*kafka.Message) error) (err error) {
	...
	topics := []string{topic}
	if kc.config.IsClose {
		gflogger.Infof(ctx, "batchSubscribeTopics consumer: %s, topics: %v (isClosed)", consumerName, topics)
		return
	}
	// 订阅topics
	var consumerList []*kafka.Consumer
...
func (kc *KafkaClient) handelData(ctx context.Context, consumerName string, consumer *kafka.Consumer, msg *kafka.Message, fn func(msg *kafka.Message) error) {
	...
	if fErr := fn(msg); fErr != nil {
		gflogger.Errorf(ctx, "%s %v consumer error: %v, topic: %v, partition: %v, offset: %v, content: %s", consumerName, consumer,
		...
		for {
			...
			if sfErr := fn(msg); sfErr != nil {
				gflogger.Errorf(ctx, "%s %v consumer error: %v, count: %d, topic: %v, partition: %v, offset: %v, content: %s", consumerName, consumer,
...
func (kc *KafkaClient) handelBatchData(ctx context.Context, consumerName string, consumer *kafka.Consumer, msgList []*kafka.Message, fn func(messages []*kafka.Message) error) {
	...
	if fErr := fn(msgList); fErr != nil {
		gflogger.Errorf(ctx, "%s %v consumer error: %v, topic: %v, partition: %v, offset: %v", consumerName, consumer,
		...
		for {
			...
			if sfErr := fn(msgList); sfErr != nil {
				gflogger.Errorf(ctx, "%s %v consumer error: %v, count: %d, topic: %v, partition: %v, offset: %v", consumerName, consumer,
...
func (kc *KafkaClient) sendData(ctx context.Context, producerConsumerName string, data []byte, key ...string) (err error) {
	...
	if kc.config.IsClose {
		gflogger.Debugf(ctx, "%s producer sendData(isClosed) msg: %s, data: %s", producerName, tools.MustString(msg), string(data))
		return
	}
	// 环境检测
	if list, ok := kc.config.ExcludeEnvTopicMap[kc.config.Env]; ok {
		if tools.ContainsStr(list, originTopic) {
			gflogger.Debugf(ctx, "%s producer sendData msg: %s, data: %s", producerName, tools.MustString(msg), string(data))
			return
		}
	}
	// 发送消息
	...
	if producer, ok = kc.producerMap[producerName]; !ok {
		err = gerror.Newf("sendData producer: %s, msg: %s, data: %s not found", producerName, tools.MustString(msg), string(data))
		return
	}
	if producer == nil {
		err = gerror.Newf("sendData producer: %s, msg: %s, data: %s producer nil", producerName, tools.MustString(msg), string(data))
		return
	}
