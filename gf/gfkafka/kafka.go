...
func (kc *KafkaClient) sendData(ctx context.Context, producerConsumerName string, data []byte, key ...string) (err error) {
	...
	if kc.config.IsClose {
		gflogger.Debugf(ctx, "%s producer sendData(isClosed) msg: %s, data: %s", producerName, gtkjson.MustString(msg), string(data))
		return
	}
	// 环境检测
	...
	if producer, ok = kc.producerMap[producerName]; !ok {
		err = gerror.Newf("sendData producer: %s, msg: %s, data: %s not found", producerName, gtkjson.MustString(msg), string(data))
		return
	}
	if producer == nil {
		err = gerror.Newf("sendData producer: %s, msg: %s, data: %s producer nil", producerName, gtkjson.MustString(msg), string(data))
		return
	}
